data "aws_s3_bucket" "vpc_flow_bucket" {
  bucket = "${local.bucket}-vpc-flow-logs"
}

data "aws_s3_bucket" "airflow_bucket" {
  bucket = "${local.bucket}-airflow"
}

data "aws_s3_bucket" "etl_bucket" {
  bucket = "${local.bucket}-etl"
}
...
module "vpc" {
  ...
  log_destination = data.aws_s3_bucket.vpc_flow_bucket.arn
  tags            = local.vpc_tags
}
